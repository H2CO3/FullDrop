...
@interface UIDevice (iOS_3_2)
- (int) userInterfaceIdiom;
@end


@interface DBLoginController () <UITextFieldDelegate, DBRestClientDelegate, UITableViewDataSource, UITableViewDelegate>
...
- (void)viewDidLoad {
    ...
    CGFloat createAccountHeight = UI_USER_INTERFACE_IDIOM() == 0 ? 36 : 44;
...
- (void)presentFromController:(UIViewController*)controller {
    ...
    if (UI_USER_INTERFACE_IDIOM() == 1) {
        navController.modalPresentationStyle = 2;
    }
...
- (CGFloat)tableView:(UITableView*)tableView heightForFooterInSection:(NSInteger)section {
    return UI_USER_INTERFACE_IDIOM() == 1 ? 80 : 0;
}

- (UIView*)tableView:(UITableView*)tableView viewForFooterInSection:(NSInteger)section {
    if (UI_USER_INTERFACE_IDIOM() == 0) return nil;
